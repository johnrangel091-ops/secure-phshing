...
import { Mail, Lock, Eye, EyeOff, CheckCircle, XCircle, Loader2, Play } from 'lucide-react';
import { Logo } from './Logo';
import { useAuth } from '../../lib/supabase/auth-context';

export function LoginForm() {
  const { signInWithPassword, signUp, signInWithGoogle, isConfigured, enterDemoMode } = useAuth();
    ...
    <div className="min-h-screen bg-[#050505] flex items-center justify-center p-6 relative overflow-hidden">
      ...
      <div className="relative z-10 w-full max-w-md">
        ...
        <div className="bg-gradient-to-br from-gray-900/50 to-gray-800/30 border border-cyan-500/30 rounded-2xl p-8 backdrop-blur-xl shadow-2xl">
          ...
          {/* Demo Mode Button - shown when Supabase is not configured */}
          {!isConfigured && (
            <div className="mt-6 pt-6 border-t border-yellow-500/30">
              <div className="bg-yellow-500/10 border border-yellow-500/30 rounded-xl p-4 mb-4">
                <p className="text-yellow-400 text-sm text-center">
                  Supabase no esta configurado. Puedes explorar la aplicacion en modo demo.
                </p>
              </div>
              <button
                onClick={enterDemoMode}
                disabled={isLoading}
                className="w-full py-4 bg-gradient-to-r from-yellow-600 to-orange-600 rounded-xl font-semibold text-white hover:from-yellow-500 hover:to-orange-500 transition-all duration-300 disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center gap-2"
                style={{
